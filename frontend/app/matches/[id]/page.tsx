...
export default function MatchPage({ params }: { params: { id: string } }) {
	...
	const { data: matchResponse, isLoading, error, refetch } = useMatch(matchId)
	...
	if (error) {
		return (
			<div className="bg-red-100 p-4 rounded-md text-red-700">
				...
				<Link href="/matches" className="mt-4 text-blue-600 hover:underline">
					Back to matches
				</Link>
			</div>
		)
						...
						<div>
							...
							<div className="space-y-2">
								...
								<div>
									...
									<span className={`px-2 py-1 text-xs rounded-full ${match.isCompleted ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>
						...
						{match.isCompleted && !isEditing ? (
							<div>
								...
								<div className="flex items-center justify-center space-x-8 p-4 bg-gray-50 rounded-lg">
									<div className="text-center">
										...
										<div className={`text-4xl font-bold ${match.teams[0]?.score > match.teams[1]?.score ? 'text-green-600' : ''}`}>{match.teams[0]?.score || 0}</div>
									</div>
									<div className="text-gray-400 text-2xl">vs</div>
									<div className="text-center">
										<div className="font-medium">{match.teams[1]?.name}</div>
										<div className={`text-4xl font-bold ${match.teams[1]?.score > match.teams[0]?.score ? 'text-green-600' : ''}`}>{match.teams[1]?.score || 0}</div>
									</div>
								</div>
							</div>
						) : isEditing ? (
							<div>
								...
								<div className="flex justify-end mt-4 space-x-2">
									...
									<Button onClick={handleSaveScores} disabled={updateMatch.isPending}>
								...
								return (
									...
									>
										<div className="flex items-center justify-between mb-3">
											<h3 className={`font-bold flex items-center gap-2 ${colorStyles.textColor}`}>
												<TeamShirt color={team.color} size="md" animated />
												{team.name}
											</h3>
											<div className={`text-sm ${colorStyles.textColor}`}>Avg Rating: {typeof team.averageRating === 'number' ? team.averageRating.toFixed(1) : '-'}</div>
										</div>
										<div className="grid grid-cols-2 gap-2">
											{team.players &&
												team.players.map(player => (
													<div key={player.playerId} className={`text-sm ${colorStyles.textColor}`}>
														{player.name} <span className="opacity-75">({player.rating})</span>
													</div>
												))}
										</div>
									</div>
