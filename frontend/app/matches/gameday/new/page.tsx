		...
		<div
			...
			<div
				...
				<div className='text-center mb-4'>
					<div
						className='inline-block h-12 w-12 rounded-full border-2 border-gray-300 mb-2'
						style={{backgroundColor: selectedTeam?.color}}></div>
						...
						return (
							<button
								...
								className={`
									...
									${
										color.value === selectedTeam?.color
...
export default function NewGameDayPage() {
	...
	const handlePlayerSelection = (playerId: string, teamIndex: number) => {
		if (
			...
			selectedPlayerForMove.playerId === playerId &&
			selectedPlayerForMove.teamIndex === teamIndex
		) {
				...
				<>
					<div className='mb-6'>
						...
						<p className='text-gray-600 mb-4'>
							בחר את השחקנים שמגיעים ליום המשחקים. לאחר מכן המערכת תיצור קבוצות מאוזנות באופן אוטומטי.
						</p>
										...
										{availablePlayers.map((player) => (
											<tr
												...
												className={`hover:bg-gray-50 cursor-pointer ${
													selectedPlayers.includes(player._id) ? 'bg-green-50' : ''
												}`}
			...
			return (
				<>
					<div className='mb-6'>
						...
						<p className='text-gray-600 mb-4'>
							המערכת חילקה את השחקנים באופן אוטומטי. ניתן לערוך את ההרכב על ידי לחיצה על שחקן אחד ולאחר
							מכן על שחקן או קבוצה אחרת.
							{selectedPlayerForMove && (
								<span className='block mt-2 text-orange-600 font-medium'>
									בחרת שחקן להחלפה.
									{transferMode === 'swap'
										? ' בחר שחקן מקבוצה אחרת להחלפה,'
										: ' בחר קבוצה אחרת להעברת השחקן,'}
								...
								<div
									...
									onClick={() => {
										if (
											...
											movePlayerToTeam(
												selectedPlayerForMove.playerId,
												selectedPlayerForMove.teamIndex,
												teamIndex
											);
										}
									}}>
										...
										{team.players?.map((player) => (
											<li
												...
												className={`flex justify-between items-center text-sm p-2 rounded-md cursor-pointer hover:bg-gray-50 ${
													selectedPlayerForMove &&
													selectedPlayerForMove.playerId === player.playerId &&
													selectedPlayerForMove.teamIndex === teamIndex
				...
				<div className='flex justify-between'>
					<div
						className={`flex-1 text-center ${
							step === 'info' ? 'font-medium text-green-800' : 'text-gray-500'
						}`}>
						פרטי יום משחקים
					</div>
					<div
						className={`flex-1 text-center ${
							step === 'players' ? 'font-medium text-green-800' : 'text-gray-500'
						}`}>
						בחירת שחקנים
					</div>
					<div
						className={`flex-1 text-center ${
							step === 'teams' ? 'font-medium text-green-800' : 'text-gray-500'
						}`}>
						קבוצות
					</div>
				</div>
				<div className='relative mt-2'>
					<div className='overflow-hidden h-2 mb-4 text-xs flex rounded bg-gray-200'>
						<div
							...
							className='shadow-none flex flex-col text-center whitespace-nowrap text-white justify-center bg-green-600 transition-all duration-300'></div>
					</div>
				</div>
			</div>
