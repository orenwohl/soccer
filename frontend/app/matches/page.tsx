...
export default function MatchesPage() {
	...
	const confirmDelete = async () => {
		...
		try {
			...
			const response = await matchService.delete(gameToDelete)
	...
	const getCompletionStatus = (gameDay: GameDay) => {
		...
		const percentage = totalMatches === 0 ? (gameDay.isCompleted ? 100 : 0) : Math.floor((completedMatches / totalMatches) * 100)

		return {
			percentage,
			text: gameDay.isCompleted ? 'הושלם' : totalMatches === 0 ? 'טרם נקבעו משחקים' : `${completedMatches}/${totalMatches} משחקים`,
	...
	if (error) {
		return (
			<div dir="rtl" className="bg-red-100 p-4 rounded-md text-red-700">
	...
	return (
		<div dir="rtl">
				...
				<div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center">
					<div className="bg-white rounded-lg p-6 max-w-md w-full mx-4 shadow-xl">
						...
						<div className="flex justify-end">
							<button onClick={closeErrorModal} className="bg-red-600 text-white px-4 py-2 rounded-md hover:bg-red-700 transition-colors">
								סגור
							</button>
						</div>
					</div>
				...
				<div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center">
					<div className="bg-white rounded-lg p-6 max-w-md w-full mx-4 shadow-xl">
						...
						<div className="flex justify-end space-x-3">
							<button onClick={cancelDelete} className="bg-gray-200 text-gray-800 px-4 py-2 rounded-md hover:bg-gray-300 transition-colors ml-3">
								ביטול
							</button>
							<button
			...
			<div className="flex justify-between items-center mb-6">
				...
				<Link href="/matches/gameday/new" className="bg-green-700 hover:bg-green-800 text-white px-4 py-2 rounded-md transition-colors">
					צור יום משחקים חדש
				</Link>
			</div>
			...
			{gamedays.length === 0 ? (
				<div className="bg-white rounded-lg shadow p-6 text-center">
					...
					<Link href="/matches/gameday/new" className="inline-block bg-green-700 hover:bg-green-800 text-white px-4 py-2 rounded-md transition-colors">
				...
				<div className="grid gap-6">
					{gamedays.map(gameday => {
						...
						return (
							<div key={gameday._id} className="relative bg-white rounded-lg shadow hover:shadow-md transition-shadow border-l-4 border-green-600">
								<Link href={`/matches/gameday/${gameday._id}`} className="block p-6">
									<div className="flex flex-col md:flex-row md:justify-between md:items-center">
										<div>
											<h2 className="text-xl font-semibold">יום משחקים - {formatDate(gameday.date)}</h2>
											<div className="text-gray-600 mt-1">{gameday.location}</div>
											<div className="mt-2 flex flex-wrap gap-2">
												{gameday.teams.map((team, idx) => (
													<span
														key={idx}
														className="inline-flex items-center gap-1.5 px-3 py-1 text-sm rounded-full border"
														style={{
															backgroundColor: team.color ? `${team.color}10` : '#f3f4f6',
															borderColor: team.color || '#d1d5db'
														}}
													>
														<TeamShirt color={team.color} size="sm" animated />
														<span style={{ color: team.color || '#1f2937' }}>{team.name}</span>
													</span>
												))}
											</div>
										</div>
										<div className="mt-4 md:mt-0">
											<div className="flex items-center">
												<div className="relative w-48 h-2 bg-gray-200 rounded">
													<div className={`absolute top-0 left-0 h-2 rounded ${status.color}`} style={{ width: `${status.percentage}%` }}></div>
												</div>
												<span className="ml-2 text-sm font-medium">{status.text}</span>
											</div>
											<div className="mt-2 text-sm text-gray-500">{gameday.matches?.length ? `${gameday.matches.length} משחקים כולל` : 'טרם נקבעו משחקים'}</div>
										</div>
									</div>
								</Link>
								<button
