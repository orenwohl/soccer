...
export default function PlayersList({players}: PlayersListProps) {
	...
	const confirmDelete = async () => {
		...
		try {
			...
			const response = await playerApi.delete(playerToDelete._id);
				...
				<div className='fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center'>
					<div className='bg-white rounded-xl shadow-xl p-6 max-w-md w-full mx-4 border border-gray-100 animate-in fade-in slide-in-from-bottom-5'>
						...
						<p className='mb-6 text-gray-600'>
							...
							<span className='font-semibold text-gray-900'>{playerToDelete.name}</span>?
						</p>
			...
			<div className='bg-white overflow-hidden rounded-xl border border-gray-200 shadow-sm'>
					...
					<Table>
						<TableHeader className='bg-gray-50 sticky top-0 z-10'>
							<TableRow className='hover:bg-gray-50/80'>
								...
								<TableHead className='text-center py-3.5 font-semibold text-gray-700 w-[180px]'>
									פעולות
								</TableHead>
							</TableRow>
						</TableHeader>
							...
							{filteredPlayers.map((player) => (
								<TableRow
									...
									<TableCell className='font-medium py-4 pl-6'>
										...
										{player.phone && (
											<div className='text-gray-500 text-sm hidden sm:block mt-0.5'>
												{player.phone}
											</div>
										)}
									</TableCell>
									...
									<TableCell className='hidden md:table-cell py-4'>
										<div className='flex flex-row items-center gap-2'>
											...
											<div className='flex items-center gap-1 bg-green-50 px-2 py-1 rounded-md'>
												...
												<span className='text-xs font-medium text-green-700 mr-1.5'>
													טכניקה:
												</span>
				...
				<div className='p-4 bg-gray-50 border-t border-gray-100 text-sm text-gray-500 flex justify-between items-center'>
					...
					<div className='text-xs'>
						דירוג ממוצע:{' '}
						{(
							filteredPlayers.reduce((sum, player) => sum + player.rating, 0) / filteredPlayers.length
						).toFixed(1)}
					</div>
				</div>
			</div>
