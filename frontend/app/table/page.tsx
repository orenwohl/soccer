...
export default function TablePage() {
	...
	const { data: leagueTableData } = useLeagueTable()
	...
	const handleGameDayChange = async (event: React.ChangeEvent<HTMLSelectElement>) => {
		...
		try {
			...
			const statsResponse = await matchService.getStatistics(gameDayId)
			...
			if (statsResponse.success && statsResponse.data) {
				...
				if (statsResponse.data.statistics && Array.isArray(statsResponse.data.statistics) && statsResponse.data.statistics.length > 0) {
				...
				// If statistics array is empty but we have game results, calculate statistics from game results
				else if (statsResponse.data.gameResults && Array.isArray(statsResponse.data.gameResults) && statsResponse.data.gameResults.length > 0) {
					...
					const gameDayResponse = await matchService.getById(gameDayId)
	...
	return (
		<div dir="rtl">
			<div className="mb-6">
				...
				<p className="text-gray-600">{selectedGameDay ? 'טבלת דירוג ליום משחק ספציפי' : 'דירוג נוכחי על סמך תוצאות המשחקים'}</p>
			</div>
			...
			{gamedays.length > 0 && (
				<div className="mb-6">
					<div className="flex flex-col md:flex-row items-start md:items-center gap-2 md:gap-4">
						...
						<div className="w-full md:w-64">
							...
							>
								{gamedays.map(gameday => (
									<option key={gameday._id} value={gameday._id}>
						...
						{selectedGameDay && (
							<button onClick={() => setSelectedGameDay(null)} className="text-sm text-blue-600 hover:text-blue-800 hover:underline">
								הצג טבלה עדכנית
							</button>
						)}
					</div>
			...
			{teams.length === 0 && !showLoading && !errorMessage ? (
				<div className="text-center p-10 bg-gray-50 rounded-lg border border-gray-200">
					...
					{selectedGameDay && (
						<button onClick={() => setSelectedGameDay(null)} className="mt-4 text-sm text-blue-600 hover:text-blue-800 hover:underline">
							הצג טבלה עדכנית
						</button>
					)}
				</div>
