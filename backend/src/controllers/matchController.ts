...
export const getMatch = async (req: Request, res: Response): Promise<void> => {
	try {
		...
		if (match.user && (!req.user || match.user.toString() !== req.user.id)) {
			console.log('getMatch: Auth failed - match.user:', match.user.toString(), 'req.user.id:', req.user ? req.user.id : 'undefined')
			res.status(401).json({
				success: false,
...
export const updateMatch = async (req: Request, res: Response): Promise<void> => {
	try {
		...
		const updatedMatch = await collection.findOneAndUpdate({ _id: new ObjectId(req.params.id) }, { $set: req.body }, { returnDocument: 'after' })
