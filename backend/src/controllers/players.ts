...
export const createPlayer = async (req: Request, res: Response): Promise<void> => {
	try {
		...
		const existingPlayer = await collection.findOne({ email: req.body.email })
		if (existingPlayer) {
			res.status(400).json({
				success: false,
...
export const updatePlayer = async (req: Request, res: Response): Promise<void> => {
	try {
		...
		const updatedPlayer = await collection.findOneAndUpdate({ _id: new ObjectId(req.params.id) }, { $set: req.body }, { returnDocument: 'after' })
